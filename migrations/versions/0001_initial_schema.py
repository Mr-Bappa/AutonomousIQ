...
def upgrade() -> None:
    ...
    dataset_visibility = postgresql.ENUM(
        "tenant_wide",
        "restricted",
        name="dataset_visibility",
        create_type=False,
    )
    dataset_visibility.create(
        op.get_bind(),
        checkfirst=True,
    )
    ...
    grant_resource_type.create(
        op.get_bind(),
        checkfirst=True,
    )

    granted_via = postgresql.ENUM(
        "team",
        "user",
        name="granted_via",
        create_type=False,
    ...
    op.create_index(
        "ix_resource_workspace_access_workspace_id",
        "resource_workspace_access",
        ["workspace_id"],
    )
    ...
    execution_target = postgresql.ENUM(
        "cloud",
        "local",
        name="execution_target",
        create_type=False,
    )
    execution_target.create(
        op.get_bind(),
        checkfirst=True,
    )
    ...
    op.create_index(
        "ix_transformation_jobs_dataset_id",
        "transformation_jobs",
        ["dataset_id"],
    )

    op.create_index(
        "ix_transformation_jobs_cache_key",
        "transformation_jobs",
        ["cache_key"],
    )
    ...
    op.create_index(
        "ix_tickets_workspace_id",
        "tickets",
        ["workspace_id"],
    )

    tenant_request_status = postgresql.ENUM(
        "pending",
        "approved",
        "rejected",
...
def downgrade() -> None:
    ...
    op.execute("DROP TYPE IF EXISTS tenant_request_status")
